...
1. `payments.getStarGifts` — каталог типов подарков, `gift_id` и `resell_min_stars`
2. Коллекции с полом **выше** `MAX_PRICE` не сканируются. Дешёвые коллекции, у которых даже лот за `MIN_PRICE` не пройдёт `MAX_MARKET_RATIO`, смотрятся по кругу (`SECONDARY_GIFT_TYPES_PER_SCAN`), а не каждый цикл
3. Для горячих коллекций вызывается `payments.getResaleStarGifts` с `stars_only=true`
4. `sort_by_price` и `sort_by_num` **не** используются в LIVE: Telegram тогда отдаёт лоты по Unix-времени последнего изменения цены (убывание) — это приоритет для свежих объявлений
5. Пагинация LIVE идёт по `next_offset`, пока не встретится уже известный лот, не кончится лента или лимит `MAX_PAGES_PER_GIFT` (по умолчанию 6). Так пачка дешёвых новинок на первой странице не прячет лот 5–25k на следующей
6. Первый прогон — **snapshot**: текущий рынок сохраняется как `EXISTING` и **не публикуется**. Потом режим `LIVE`: в канал идут только лоты, которых не было на рынке в момент старта. Новый `gift_id`, которого ещё нет в БД, сначала запоминается как snapshot
7. Цена лота сверяется с медианой коллекции (`MAX_MARKET_RATIO=3`). Подарок за 300⭐, выставленный за 7к, не пройдёт. Оценку рынка берём со страниц скана, без лишнего круга API
8. Один владелец публикуется один раз. Одна и та же модель/коллекция не идёт подряд
9. Мужской профиль отсекается по имени со страницы resale **до** `GetFullUser` / подсчёта NFT. Если у UserFull `stargifts_count ≤ 12`, отдельный обход `getSavedStarGifts` не нужен
...
- Queue, last scan/publish, среднее время скана, catalog `hot/watch/skip`, scanned, new, filtered, skip-причины, sent, errors
