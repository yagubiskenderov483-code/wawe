...
- floor выше `COLLECTION_FLOOR_MAX` → коллекция пропускается: даже её самый дешёвый лот дороже потолка
- floor ниже `COLLECTION_FLOOR_MIN` → пропуск, но по умолчанию `COLLECTION_FLOOR_MIN=0`, то есть нижнего отсечения нет
- floor неизвестен → коллекция сканируется

Важно: `resell_min_stars` — это **самый дешёвый** лот коллекции, а не её типичная цена. Коллекция с полом 626⭐ спокойно содержит лоты по 15000⭐, поэтому отсекать по нижней границе нельзя — так теряется большая часть рынка. Дешёвые лоты стоят дёшево и по трафику: цена проверяется локально, без единого запроса к Telegram.
...
## 12a. MEMORY_ONLY — работа без базы на диске

`MEMORY_ONLY=true` (по умолчанию) — SQLite поднимается в памяти процесса, на диск ничего не пишется, бэкапы не делаются.

Как это работает при каждом запуске:

1. Сканер молча запоминает весь текущий рынок (snapshot), в канал ничего не идёт
2. Дальше публикуются только лоты, появившиеся **после** старта

Смысл в том, что базы, пережившей рестарт, больше нет — а значит нет и старых лотов, которые она могла бы догнать. `PUBLISH_EXISTING=true` в этом режиме принудительно выключается: без памяти между запусками он отправил бы весь рынок заново после каждого рестарта.

Обратная сторона: ручные метки `/tag` живут только до перезапуска, истории цен нет. Если нужны — `MEMORY_ONLY=false`, тогда всё как раньше, в `data/tracker.db`.
...
- `STRICT_FEMALE_NAME=true` (по умолчанию) — самая жёсткая проверка: нужен либо `/tag ... gender=female`, либо имя из списка женских. Женская фамилия (-ова/-ева/-ина) сама по себе больше не пропускает лот
