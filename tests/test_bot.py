...
class FakeBot:
    def __init__(self, me=None, error=None, chats=None):
        self._me = me
        self._error = error
...
class VerifyTargetChannelsTests(unittest.IsolatedAsyncioTestCase):
    async def test_unauthorized_token_is_configuration_error(self):
        bot = FakeBot(error=TelegramUnauthorizedError(method=GetMe(), message="Unauthorized"))
        cfg = settings(bot_token="8825465611:AAHrevokedtokenvalue000", target_channel_id=8825465611)
    ...
    async def test_self_id_target_skips_get_chat(self):
    ...
    async def test_missing_targets_fail_fast(self):
